...
function Search({searchResults: {users, groups, channels}, loading, setSearchInput}) {
  const renderUserResults = useMemo(() => {
    return (
      <ul className="flex flex-col gap-2">
        <div className="px-3 text-lg">Users</div>
        {users.map(user => (
          <SearchResult key={user.id} account={user} type="user" setSearchInput={setSearchInput} />
        ))}
      </ul>
    )
  ...
  return (
    <div className="flex flex-col bg-zinc-950 shadow-lg p-4 rounded-lg w-full h-full overflow-hidden">
      ...
      <section className="flex flex-col gap-4 overflow-y-auto grow">
        ...
        {users.length === 0 && groups.length === 0 && channels.length === 0 && (
          <p className="my-10 text-center text-lg">{loading ? "Loading..." : "No results found"}</p>
        )}
      </section>
    </div>
  )
...
const Memoized = React.memo(Search)
