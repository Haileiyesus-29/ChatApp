...
function SearchResult({type = "user", account, setSearchInput}) {
  ...
  const onJoin = async () => {
    if (type === "group") {
      await joinGroup({groupId: account.id}, () => navigate(paths.groupMessage(account.id)))
      setSearchInput("")
    }
    if (type === "channel") {
      await joinChannel({channelId: account.id}, () => navigate(paths.channelMessage(account.id)))
      setSearchInput("")
    }
  }
