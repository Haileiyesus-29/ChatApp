...
function PersonalChatBubble({message}) {
  ...
  if (message.type === "date")
    return <span className="bg-zinc-950 px-6 py-1 rounded-full text-sm self-center">March 28</span>
      ...
      <div className="flex items-end gap-2">
        <Link to={paths.chatMessage(message.user.id)} className="py-1">
          <Avatar className="w-10 h-10">
            <AvatarImage
              src={message.user.image || "default_profile.jpg"}
        ...
        <div className="">
          <h4 className="font-semibold text-sm text-zinc-300/80">{message.user.name}</h4>
          <div className={setClassName(sent)}>
            <p className="grow">{message.text}</p>
            <span className={`text-xs ${sent ? "text-zinc-900/80" : "text-zinc-200/80"}`}>
